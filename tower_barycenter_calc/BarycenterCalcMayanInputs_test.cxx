...
std::vector<float> InputStreamEta_Vec = {-2.4}; // Single value
...
for (int i = 0; i < NCells+1; ++i) {
    ...
    BCCalcStreamMayanInputs(InputStreamEt, InputStreamEta, InputStreamBitMask, OutputStream_Test);
}

if (!OutputStream_Test.empty()) {
    fBC OutBC = OutputStream_Test.read();
    std::cout << "Output BC = " << OutBC << std::endl;
} else {
