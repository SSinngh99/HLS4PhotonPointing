...
hls::stream<fET> OutStreamMaxET_Test;
...
std::vector<float> InputStreamEt_Vec =     {-3, 10, 14, 3,
                                            2,  1,  20, 9,
                                            -1, -2}; //  < MaxNCellsL1 for now, and TestV1
std::vector<float> InputStreamEta_Vec = {1.0,      1.003125, 1.009375, 1.0025, 
                                         1.04375,  1.046875, 1.0625,   1.078125, 
                                         1.096875, 1.053125};
std::vector<bool> InputStreamBitMask_Vec = {0, 1, 1, 1, 
                                            1, 1, 0, 1, 
                                            1, 0}; // TestV1, 14 and 1.009375 for raw eta (note 20 is flagged as bad)



// Fill up ET to 16 to not consider stuff beyond the things written above
for (int i = InputStreamEt_Vec.size(); i < MaxNCellsL1; ++i){
  InputStreamBitMask_Vec.push_back(0);
  InputStreamEta_Vec.push_back(-999);
}

for (int i = 0; i < MaxNCellsL1; ++i) {
...
for (int i = 0; i < MaxNCellsL1+1; ++i) {
    std::cout << "Running FindMaxCellL1, idx = " << i << std::endl;
    FindMaxCellL1(InputStreamEt, InputStreamEtaIdxL1, InputStreamBitMask, OutStreamEtaIdxL1_Test, OutStreamMaxET_Test);
}

if (!OutStreamEtaIdxL1_Test.empty()) {
    fEtaIdxL1 OutEtaIdx = OutStreamEtaIdxL1_Test.read();
    fET OutET = OutStreamMaxET_Test.read();
    std::cout << "Output EtaIdx = " << OutEtaIdx << ", OutET = " << OutET << std::endl;
} else {
